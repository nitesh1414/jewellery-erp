          ...
          <tbody>
            ...
            {list.map((e) => (
              <tr key={e.id} className="border-b border-gray-50 hover:bg-gray-50">
                ...
                <td className="table-cell text-[13px]">{e.vendor || '—'}</td>
                <td className="table-cell text-[13px] text-gray-700">{e.description || '—'}</td>
                <td className="table-cell text-[13px] text-gray-500">{e.paidFromMode || 'CASH'}</td>
                <td className="table-cell text-right font-semibold text-red-600">− {fmtMoney(e.amount)}</td>
                <td className="table-cell text-right">
                  <button className="btn-ghost p-1 text-red-500" onClick={async () => { if (await confirmAction({ title: 'Delete this expense?', message: 'The ledger entry made for it will be reversed.', danger: true, confirmLabel: 'Delete' })) deleteMut.mutate(e.id); }}><Trash2 className="w-3.5 h-3.5" /></button>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50" onClick={() => setShowForm(false)}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4 p-3 sm:p-4 modal-panel" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-base font-semibold mb-3">New Expense</h3>
            <div className="space-y-3">
